...
export const fetchCurrentWeather = async (lat: number, lng: number, units: Units) => {
  ...
  export const fetchForecast = async (lat: number, lng: number, units: Units): Promise<ForecastItem[]> => {
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL;
    const response = await fetch(
  ...
  export const fetchAirQuality = async (lat: number, lng: number, units: Units) => {
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL;
    const response = await fetch(
