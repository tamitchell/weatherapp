...
import { fetchCurrentWeather, fetchForecast, fetchAirQuality } from "./weatherCalls";
...
describe('Weather API functions', () => {
    ...
    describe('fetchCurrentWeather', () => {
      it('fetches and returns current weather data', async () => {
        ...
        const result = await fetchCurrentWeather(40.7128, -74.0060, 'imperial');
        expect(result).toEqual(mockWeather);
        expect(fetchMock).toHaveBeenCalledWith(
          expect.stringContaining('/api/weather?lat=40.7128&lng=-74.006&units=imperial')
        );
      });
      ...
      it('throws error on failed fetch', async () => {
        ...
        await expect(
          fetchCurrentWeather(40.7128, -74.0060, 'imperial')
    ...
    describe('fetchForecast', () => {
        it('fetches and returns only the forecast list array', async () => {
          ...
          const result = await fetchForecast(40.7128, -74.0060, 'imperial');
          ...
          expect(fetchMock).toHaveBeenCalledWith(
            expect.stringContaining('/api/weather/forecast?lat=40.7128&lng=-74.006&units=imperial')
          );
        });
        ...
        it('throws error on failed fetch', async () => {
          ...
          await expect(
            fetchForecast(40.7128, -74.0060, 'imperial')
        ...
        it('throws error on malformed response - missing list', async () => {
          ...
          await expect(
            fetchForecast(40.7128, -74.0060, 'imperial')
          ).rejects.toThrow('Invalid forecast data structure');
        });
    
        it('throws error on malformed response - list is not an array', async () => {
          const malformedResponse = { cod: '200', message: 0, list: 'not an array' };
          fetchMock.mockResponseOnce(JSON.stringify(malformedResponse));
    ...
    describe('fetchAirQuality', () => {
      it('fetches and returns air quality data', async () => {
        ...
        const result = await fetchAirQuality(40.7128, -74.0060, 'imperial');
        expect(result).toEqual(mockAirQuality);
        expect(fetchMock).toHaveBeenCalledWith(
          expect.stringContaining('/api/weather/air_pollution?lat=40.7128&lng=-74.006&units=imperial')
        );
      });
      ...
      it('throws error on failed fetch', async () => {
        ...
        await expect(
          fetchAirQuality(40.7128, -74.0060, 'imperial')
