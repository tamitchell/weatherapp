import useOutfitRecommendationQuery, { OutfitRecommendationQueryProps } from "src/hooks/queries/useOutfitRecommendationQuery/useOutfitRecommendationQuery";
...
export default memo(function OutfitRecommendation({  chanceOfPrecip, currentWeather, forecast, units,}: OutfitRecommendationProps) {
    const { data, isLoading, error } = useOutfitRecommendationQuery({chanceOfPrecip, currentWeather, forecast, units});  
    ...
    return (
      ...
      >
        ...
        <div className="">
         ...
         { error || !data ? <p>Unable to load clothing recommendation</p> : <WordReveal text={data.recommendation} />}
        </div>
      </div>
    );
  });